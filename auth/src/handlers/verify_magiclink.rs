...
use crate::state::{ConfirmationFlow, ConfirmationRecord, TokenRecord};
...
pub async fn verify_magiclink_code_handler(
    ...
    let mut confirmation: ConfirmationRecord = serde_json::from_slice(&confirmation_bytes)
        .unwrap_or_else(|_| {
    ...
    let access_token_details =
        generate_paseto_token(&user_id, TokenKind::Access, &auth_state).unwrap();

    let refresh_token_details =
        generate_paseto_token(&user_id, TokenKind::Refresh, &auth_state).unwrap();
    ...
    auth_state
        ...
        .kv_insert(
            a_key.as_bytes(),
            &serde_json::to_vec(&access_token_record).unwrap(),
        )
        .unwrap();
    auth_state
        .store
        ...
        .kv_insert(
            r_key.as_bytes(),
            &serde_json::to_vec(&refresh_token_record).unwrap(),
        )
        .unwrap();
    ...
    access_cookie.set_max_age(time::Duration::minutes(
        auth_state.config.access_token_ttl_minutes,
    ));
    ...
    refresh_cookie.set_max_age(time::Duration::days(
        auth_state.config.refresh_token_ttl_days,
    ));
