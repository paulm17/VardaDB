...
use crate::state::{TokenRecord, UserRecord};
...
pub async fn login_user_handler(
    ...
    let access_token_details = generate_paseto_token(&user.id, TokenKind::Access, &auth_state)
        .map_err(|e| {
            tracing::error!("Access token gen failed: {:?}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({"status": "fail", "message": "Internal error"})),
            )
        })?;

    let refresh_token_details = generate_paseto_token(&user.id, TokenKind::Refresh, &auth_state)
        .map_err(|e| {
