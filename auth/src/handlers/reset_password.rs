...
pub async fn reset_password_handler(
    ...
    let mut user: UserRecord = serde_json::from_slice(&user_bytes)
        .unwrap_or_else(|_| bincode::deserialize(&user_bytes).expect("Failed to deserialize user"));
    ...
    auth_state
        .store
        .users
        .kv_insert(user_key.as_bytes(), &serde_json::to_vec(&user).unwrap())
        .map_err(|e| {
    ...
    let mut response = Response::new(serde_json::json!({"status": "ok"}).to_string());
