				...
				<Box className={classes.wrapper}>
					...
					>
						...
						<Switch>
							...
							<Route path="/management/mini/new">
								<NewEmbedPageLazy />
							</Route>

							<Route path="/management/connections/create">
								<CreateConnectionPageLazy />
							</Route>

							<Route path="/management/support">
								<SupportPageLazy />
							</Route>

							<Route path="/management/support/collections/:collection">
								{({ collection }) => <CollectionPage id={collection} />}
							</Route>

							<Route path="/management/support/articles/:article">
								{({ article }) => <ArticlePage id={article} />}
							</Route>

							<Route path="/management/support/requests">
								<RequestsPageLazy />
							</Route>

							<Route path="/management/support/conversations/:conversation">
								{({ conversation }) => <ConversationPage id={conversation} />}
							</Route>

							{showCloud && (
								<>
									<Route path="/management/organisations/create">
										<CreateOrganizationsPageLazy />
									</Route>

									<Route path="/management/organisations">
										<OrganizationsPageLazy />
									</Route>

									<Route path="/management/o/:organization/deploy">
										{({ organization }) => (
											<OrganizationDeployPageLazy id={organization} />
										)}
									</Route>

									<Route path="/management/o/:organization/support-plans">
										{({ organization }) => (
											<SupportPlansPageLazy id={organization} />
										)}
									</Route>

									<Route path="/management/o/:organization/:tab">
										{({ organization, tab }) => (
									...
									<Route path="/management/o/:organization">
										{({ organization }) => (
											<Redirect to={`/management/o/${organization}/instances`} />
										)}
									</Route>

									<Route path="/management/referrals">
										<ReferralPageLazy />
									</Route>

									<Route path="/management/signin/:plan?">
										{({ plan }) => <SigninPageLazy plan={plan} />}
									</Route>

									<Route path="/management/cloud">
										<Redirect to="/management/signin" />
									</Route>

									<Route path="/management/billing">
										<Redirect to="/management/organisations" />
									</Route>
								</>
							)}

							<Route path="/management/c/:connection/:view">
								{({ view }) => {
