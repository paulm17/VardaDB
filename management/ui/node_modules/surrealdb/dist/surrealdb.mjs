...
*/
var ResponseError = class extends SurrealError {
	name = "ResponseError";
	code;
	constructor(response) {
	...
	*/
	static parseString(input) {
		...
		while (left !== "") {
			...
			} else throw new SurrealError("Could not match a next duration part");
		}
...
var Decimal = class Decimal extends Value {
	...
	*/
	static fromScientificNotation(input) {
		...
		if (!/^[+-]?\d+(\.\d+)?[eE][+-]?\d+$/.test(trimmed)) throw new Error(`Invalid scientific notation: ${input}`);
...
var RecordId = class RecordId extends Value {
	...
	constructor(table, id$1) {
		...
		if (!isValidIdPart(id$1)) throw new SurrealError("id part is not valid");
...
var StringRecordId = class StringRecordId extends Value {
	...
	constructor(rid) {
		...
		else throw new SurrealError("String Record ID must be a string");
	}
	equals(other) {
...
const knn = (field, v, neighbors, metricOrEf) => ({ toSQL: (ctx) => {
	if (!Number.isInteger(neighbors)) throw new Error("neighbors must be an integer");
	if (metricOrEf !== void 0 && !Number.isInteger(metricOrEf) && !/\w+/.test(metricOrEf)) throw new Error("metric is invalid");
...
var BoundQuery = class BoundQuery {
	...
	*/
	...
	append(other, ...values) {
		...
		if (_other.#bindings) {
			for (const key of Object.keys(_other.#bindings)) if (key in this.#bindings) throw new Error(`Parameter conflict: '${key}' already exists in this BoundQuery`);
...
const _only = (value) => ({ toSQL: (ctx) => value instanceof _RecordId ? `ONLY ${ctx.def(value)}` : ctx.def(value) });
const _output = (value) => ({ toSQL: () => {
	const output = OUTPUTS.get(value);
	if (!output) throw new Error(`Invalid output value: ${value}`);
	return output;
} });
const _timeout = (timeout) => ({ toSQL: () => {
	if (!(timeout instanceof Duration)) throw new Error(`Invalid timeout value: ${timeout}`);
...
var RelatePromise = class RelatePromise extends DispatchedPromise {
	...
	*/
	...
	#build() {
		...
		if (isMultiple && what instanceof _RecordId) throw new Error("Edge must be a table when creating multiple edges");
...
var RunPromise = class RunPromise extends DispatchedPromise {
	...
	*/
	...
	#build() {
		...
		if (!NAME_REGEX.test(name)) throw new SurrealError("Invalid function name");
		const query = new BoundQuery(name);
		if (version) {
			if (!VERSION_REGEX.test(version)) throw new SurrealError("Invalid function version");
...
var ManagedLiveSubscription = class extends LiveSubscription {
	...
	[Symbol.asyncIterator]() {
		if (this.#killed) throw new SurrealError("Subscription has been killed");
...
async function fetchSurreal(context, state, session, options) {
	...
	const raw$1 = await fetchImpl(endpoint, {
		method: options.method ?? "POST",
		headers: headerMap,
		body: options.body ? context.codecs.cbor.encode(options.body) : void 0
	});
	const buffer = await raw$1.arrayBuffer();
	if (raw$1.status === 200) return new Uint8Array(buffer);
	const dec = new TextDecoder("utf-8");
	throw new HttpConnectionError(dec.decode(buffer), raw$1.status, raw$1.statusText, buffer);
}
...
function buildRpcAuth(session, auth) {
	...
	if ("variables" in auth) {
		...
		if (!database$1 || !namespace$1) throw new SurrealError("Namespace and database must be provided or selected for record authentication");
		return {
			...auth.variables,
			ac: auth.access,
			ns: namespace$1,
	...
	if (database && !namespace) throw new SurrealError("Database authentication requires a namespace to be provided");
...
var HttpEngine = class extends RpcEngine {
	...
	async send(request) {
		...
		const buffer = await fetchSurreal(this._context, this._state, session, { body: {
			id: id$1,
			...request
		} });
		let response;
		...
		if (response.error) throw new ResponseError(response.error);
	...
	handleRpcResponse({ id: id$1,...res }) {
		if (typeof id$1 === "string") {
			try {
				...
				if (response.error) reject?.(new ResponseError(response.error));
...
var ConnectionController = class {
	...
	async use(what, session) {
		if (!this.#engine || !this.#state) throw new ConnectionUnavailableError();
		if (what.namespace === null && what.database !== null) throw new SurrealError("Cannot unset namespace without unsetting database");
...
//#endregion
export { AuthenticationError, BoundExcluded, BoundIncluded, BoundQuery, CallTerminatedError, CborCodec, ChannelIterator, ConnectionUnavailableError, DateTime, Decimal, DoneFrame, Duration, ErrorFrame, ExpressionError, Features, FileRef, Frame, Future, Geometry, GeometryCollection, GeometryLine, GeometryMultiLine, GeometryMultiPoint, GeometryMultiPolygon, GeometryPoint, GeometryPolygon, HttpConnectionError, HttpEngine, InvalidDateError, InvalidSessionError, LIVE_ACTIONS, LiveSubscription, LiveSubscriptionError, MAXIMUM_VERSION, MINIMUM_VERSION, ManagedLiveSubscription, MissingNamespaceDatabaseError, PublishError, Publisher, Range, ReconnectExhaustionError, ReconnectIterationError, _RecordId as RecordId, _RecordIdRange as RecordIdRange, ResponseError, RpcEngine, StringRecordId, Surreal, SurrealApi, SurrealError, SurrealQueryable, SurrealSession, SurrealTransaction, Table, UnavailableFeatureError, UnexpectedConnectionError, UnexpectedServerResponseError, UnmanagedLiveSubscription, UnsuccessfulApiError, UnsupportedEngineError, UnsupportedFeatureError, UnsupportedVersionError, Uuid, Value, ValueFrame, WebSocketEngine, and, applyDiagnostics, between, contains, containsAll, containsAny, containsNone, createRemoteEngines, d, eeq, eq, equals, escapeIdPart, escapeIdent, escapeNumber, escapeRangeBound, expr, gt, gte, inside, intersects, isVersionSupported, jsonify, knn, lt, lte, matches, not, or, outside, r, raw, s, surql, toSurqlString, toSurqlString as toSurrealqlString, u };