...
fn generate_todos(count: usize, start_id: usize) -> Vec<HashMap<String, serde_json::Value>> {
    (0..count)
        .map(|i| {
            ...
            fields.insert(
                "title".to_string(),
                serde_json::json!(format!("Todo item {}", id)),
            );
            fields.insert("completed".to_string(), serde_json::json!(id % 2 == 0));
            fields.insert(
                "priority".to_string(),
                serde_json::json!((id % 5) as i64 + 1),
            );
            fields.insert(
                "createdAt".to_string(),
                serde_json::json!(format!("2024-01-{:02}T12:00:00Z", (id % 28) + 1)),
            );
            fields
        })
        .collect()
...
fn manual_million_test() {
    ...
    for count in batch_sizes {
        ...
        println!("{}:", count);
        println!(
            "  Generation: {:?} ({:.0} items/sec)",
            gen_time,
            count as f64 / gen_time.as_secs_f64()
        );
        println!(
            "  Serialization: {:?} ({:.0} items/sec)",
            ser_time,
            count as f64 / ser_time.as_secs_f64()
        );
