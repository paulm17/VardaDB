...
fn test_cron_registration() {
    ...
    ctx.queue.register_cron("test-cron".to_string(), expr.to_string(), "default".to_string(), vec![]).unwrap();
...
fn test_cron_trigger() {
    ...
    ctx.queue.register_cron("trigger-test".to_string(), expr.to_string(), "default".to_string(), vec![]).unwrap();
