...
fn test_scheduled_promotion() {
    ...
    job1_sched.location = JobLocation::Scheduled { queue: "default".into() };
...
fn test_scheduled_ordering() {
    ...
    let mut j1 = Job::new(1, "default".into(), vec![]); j1.run_at = now + 5000;
    let mut j2 = Job::new(2, "default".into(), vec![]); j2.run_at = now + 2000;
    let mut j3 = Job::new(3, "default".into(), vec![]); j3.run_at = now + 10000;
    ...
    let promoted_count = ctx.store.promote_scheduled("default", now + 3000, 10).unwrap();
