...
async fn test_has_inverse_logic() {
    ...
    let alice_id = v1["data"]["createUser"]["id"].as_str().unwrap().to_string();

    // 3. Create Post linking to Alice (via 'author')
    // Expectations: Alice.posts should automatically contain this Post
    let m2 = format!(
        "mutation {{ createPost(input: {{title: \"Post 1\", author: \"{}\"}}) {{ id }} }}", 
        alice_id
    );
    ...
    let post_id = v2["data"]["createPost"]["id"].as_str().unwrap().to_string();

    // 4. Verify Alice has the post (Reverse Edge)
    let q1 = format!(
        "query {{ getUser(id: \"{}\") {{ posts {{ title }} }} }}",
        alice_id
    );
    ...
    let m3 = "mutation { createUser(input: {name: \"Bob\"}) { id } }";
    let r3 = schema.execute_with_resolver(m3, resolver.clone()).await;
    let v3: Value = serde_json::from_str(&r3).unwrap();
    let bob_id = v3["data"]["createUser"]["id"].as_str().unwrap().to_string();

    let m4 = format!(
        "mutation {{ updatePost(id: \"{}\", input: {{author: \"{}\"}}) }}",
        post_id, bob_id
    );
    ...
    // Verify Alice lost the post
    let q_alice = format!("query {{ getUser(id: \"{}\") {{ posts {{ title }} }} }}", alice_id);
    ...
    let q_bob = format!("query {{ getUser(id: \"{}\") {{ posts {{ title }} }} }}", bob_id);
