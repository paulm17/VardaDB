...
async fn test_has_inverse_logic() {
    ...
    let alice_id = v1["data"]["createUser"]["uid"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let post_id = v2["data"]["createPost"]["uid"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let bob_id = v3["data"]["createUser"]["uid"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let r_alice = schema
        .execute_with_resolver(&q_alice, resolver.clone())
        .await;
    ...
    if let Some(list) = alice_posts {
        if !list.is_empty() {
            panic!(
                "TEST FAIL: Alice still has the post after update! Found: {:?}",
                list
            );
        }
    }
    ...
    let r_bob_2 = schema
        .execute_with_resolver(&q_bob_2, resolver.clone())
        .await;
    ...
    if let Some(list) = bob_posts_2 {
        if !list.is_empty() {
            panic!(
                "TEST FAIL: Bob STILL has the post after deletion! Found: {:?}",
                list
            );
        }
    }
