...
async fn test_pagination_flow() {
    ...
    for i in 1..=5 {
        let mutation = format!(
            "mutation {{ createUser(input: {{name: \"User{}\"}}) {{ id name }} }}",
            i
        );
    ...
    ";
    let res_p1 = schema.execute_with_resolver(query_p1, resolver.clone()).await;
    let v_p1: Value = serde_json::from_str(&res_p1).unwrap();
    let users_p1 = v_p1["data"]["queryUser"].as_array().expect("Expected array");
    assert_eq!(users_p1.len(), 2);
    assert_eq!(users_p1[0]["name"], "User1");
    assert_eq!(users_p1[1]["name"], "User2");
    
    let cursor = users_p1[1]["id"].as_str().unwrap(); // Should be UID of User2
    ...
    let res_p2 = schema.execute_with_resolver(&query_p2, resolver.clone()).await;
    let v_p2: Value = serde_json::from_str(&res_p2).unwrap();
    let users_p2 = v_p2["data"]["queryUser"].as_array().expect("Expected array");
    assert_eq!(users_p2.len(), 2);
    assert_eq!(users_p2[0]["name"], "User3");
    assert_eq!(users_p2[1]["name"], "User4");
    
    let cursor_2 = users_p2[1]["id"].as_str().unwrap();
    ...
    let res_p3 = schema.execute_with_resolver(&query_p3, resolver.clone()).await;
    let v_p3: Value = serde_json::from_str(&res_p3).unwrap();
    let users_p3 = v_p3["data"]["queryUser"].as_array().expect("Expected array");
