...
async fn test_text_search() {
    ...
    let posts_all = res_all["data"]["queryPost"].as_array().expect("Expected array");
    ...
    ";
    let res_any_json = schema.execute_with_resolver(query_any, resolver.clone()).await;
    let res_any: Value = serde_json::from_str(&res_any_json).unwrap();
    let posts_any = res_any["data"]["queryPost"].as_array().expect("Expected array");
    ...
    ";
    let res_run_strict = schema.execute_with_resolver(query_run_strict, resolver.clone()).await;
    ...
    assert_eq!(posts_run_strict.len(), 0, "Strict search 'run' should NOT match 'running'");
    ...
    ";
    let res_run_stemmed = schema.execute_with_resolver(query_run_stemmed, resolver.clone()).await;
    ...
    assert_eq!(posts_run_stemmed.len(), 1, "Stemmed search 'run' SHOULD match 'running'");
    ...
    ";
    let res_old = schema.execute_with_resolver(query_old, resolver.clone()).await;
    let res_old_val: Value = serde_json::from_str(&res_old).unwrap();
    assert_eq!(res_old_val["data"]["queryPost"].as_array().unwrap().len(), 0);
    ...
    ";
    let res_new = schema.execute_with_resolver(query_new, resolver.clone()).await;
    let res_new_val: Value = serde_json::from_str(&res_new).unwrap();
    assert_eq!(res_new_val["data"]["queryPost"].as_array().unwrap().len(), 1);
}
