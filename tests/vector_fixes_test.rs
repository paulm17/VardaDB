...
async fn test_vector_deletion_and_dim_check() {
    ...
    let id1 = json["data"]["createItem"]["uid"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let res = schema
        .execute_with_resolver(mut_fail, resolver.clone())
        .await;
    let json: JsonValue = serde_json::from_str(&res).unwrap();
    assert!(
        json["errors"].is_null(),
        "Expected success for async insertion (validation is background)"
    );
    ...
    let res = schema
        .execute_with_resolver(&mut_del, resolver.clone())
        .await;
    let json: JsonValue = serde_json::from_str(&res).unwrap();
