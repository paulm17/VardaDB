...
async fn test_hybrid_search() {
    ...
    let results = json["data"]["hybridSearch"].as_array().expect("Result array");
    
    // Doc 1 should be top (Direct Vector Hit + Partial Text Match)
    // Doc 1 and Doc 3 might have tied scores, verify they are the top 2
    let top_uids: Vec<String> = results.iter().take(2).map(|r| r["uid"].as_str().unwrap().to_string()).collect();
