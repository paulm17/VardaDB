...
async fn test_nested_pagination() {
    ...
    let res_user = schema.execute_with_resolver(&mutation_user, resolver.clone()).await;
    ...
    ";
    let res_json = schema.execute_with_resolver(query_first_2, resolver.clone()).await;
    let res: Value = serde_json::from_str(&res_json).unwrap();
    let posts = res["data"]["queryUser"][0]["posts"].as_array().expect("Expected posts array");
    ...
    let res_next_json = schema.execute_with_resolver(&query_next_2, resolver.clone()).await;
    let res_next: Value = serde_json::from_str(&res_next_json).unwrap();
    let posts_next = res_next["data"]["queryUser"][0]["posts"].as_array().expect("Expected posts array");
    ...
    ", cursor_2);
    let res_last_json = schema.execute_with_resolver(&query_last, resolver.clone()).await;
    let res_last: Value = serde_json::from_str(&res_last_json).unwrap();
    let posts_last = res_last["data"]["queryUser"][0]["posts"].as_array().expect("Expected posts array");
