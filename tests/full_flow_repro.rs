...
async fn test_full_server_flow_repro() {
    ...
    // 1. START SERVER (Simulation) & 2. CREATE DATABASE & 3. APPLY SCHEMA
    {
        ...
        let schema_wrapper = state.schemas.get(db_name).expect("Schema should be loaded").clone();
    ...
    // 5. RESTART SERVER & 6. QUERY DATA
    {
        ...
        // Check if DB exists
        assert!(storage.get_database(db_name).is_some(), "Database should persist in storage");
