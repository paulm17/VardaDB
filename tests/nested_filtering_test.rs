...
async fn test_nested_filtering() {
    ...
    let res_users = schema
        .execute_with_resolver(&mutation_users, resolver.clone())
        .await;
    ...
    let all_users = all_data["data"]["queryUser"]
        .as_array()
    ...
    ";
    let res_go_json = schema
        .execute_with_resolver(query_go, resolver.clone())
        .await;
    let res_go: Value = serde_json::from_str(&res_go_json).unwrap();
    let users_go = res_go["data"]["queryUser"]
        .as_array()
        .expect("Expected array");
