...
async fn test_crud_flow() {
    ...
    // 2. Create User
    let m1 = "mutation { createUser(input: {name: \"Alice\", email: \"alice@example.com\"}) { id } }";
    ...
    // 3. Verify Create
    let q1 = format!("query {{ getUser(id: \"{}\") {{ name email }} }}", user_id);
    ...
    let m2 = format!(
        "mutation {{ updateUser(id: \"{}\", input: {{name: \"Alicia\"}}) }}",
        user_id
    );
    ...
    // 5. Verify Update
    let q2 = format!("query {{ getUser(id: \"{}\") {{ name email }} }}", user_id);
    ...
    // Create another user
    let m3 = "mutation { createUser(input: {name: \"Bob\", email: \"bob@example.com\"}) { id } }";
    schema.execute_with_resolver(m3, resolver.clone()).await;

    // Try to update Alice's email to Bob's
    let m4 = format!(
        "mutation {{ updateUser(id: \"{}\", input: {{email: \"bob@example.com\"}}) }}",
        user_id
    );
    ...
    // 7. Delete User
    let m5 = format!("mutation {{ deleteUser(id: \"{}\") }}", user_id);
    ...
    // 8. Verify Delete
    let q3 = format!("query {{ getUser(id: \"{}\") {{ name }} }}", user_id);
    ...
    let m6 = "mutation { createUser(input: {name: \"Alice 2\", email: \"alice@example.com\"}) { id } }";
