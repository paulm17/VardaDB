...
async fn test_crud_flow() {
    ...
    // 2. Create User
    let m1 =
        "mutation { createUser(input: {name: \"Alice\", email: \"alice@example.com\"}) { uid } }";
    ...
    assert_eq!(
        vq2["data"]["getUser"]["email"], "alice@example.com",
    ...
    let m6 =
        "mutation { createUser(input: {name: \"Alice 2\", email: \"alice@example.com\"}) { uid } }";
