...
async fn test_index_document_commits_immediately() {
    ...
    let results = search2.search_bm25("default", "test", "content", "term", 10, false);
...
async fn test_update_survives_crash() {
    ...
    let results = search2.search_bm25("default", "original", "content", "term", 10, false);
    ...
    let results_original = search3.search_bm25("default", "original", "content", "term", 10, false);
    assert_eq!(results_original.len(), 0, "Original content should be gone");

    let results_updated = search3.search_bm25("default", "updated", "content", "term", 10, false);
    assert_eq!(
        results_updated.len(),
        1,
