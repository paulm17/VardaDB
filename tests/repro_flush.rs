...
fn repro_flush_wal_truncation() {
    ...
    // 1. Open DB
    {
        ...
        for i in 0..20000 {
             ...
             if items.len() >= 100 {
                 ...
                 storage.put_batch_lww("test", items.clone(), &storage_ts).unwrap();
                 items.clear();
             }
        }
        
        println!("Writes complete. DB Size (Pre-Flush): {} MB", get_dir_size(&db_path) / 1024 / 1024);
        ...
        println!("Flush complete. DB Size (Post-Flush): {} MB", get_dir_size(&db_path) / 1024 / 1024);

        println!("Listing files in DB dir:");
        for entry in std::fs::read_dir(&db_path).unwrap() {
            let entry = entry.unwrap();
            println!("  {:?} ({} bytes)", entry.file_name(), entry.metadata().unwrap().len());
        }
    ...
    assert!(duration < Duration::from_secs(2), "Restart took too long: {:?}", duration);
}
