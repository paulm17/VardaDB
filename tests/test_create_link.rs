...
async fn test_create_link_object_input() {
    ...
    // 1. Create Parent
    let res = schema.execute_with_resolver("mutation { createParent(input: {}) { uid } }", resolver.clone()).await;
    let parent_id = serde_json::from_str::<JsonValue>(&res).unwrap()["data"]["createParent"]["uid"].as_str().unwrap().to_string();

    // 2. Create Child linked to Parent via Object input
    let query = format!("mutation {{ createChild(input: {{ parent: {{ uid: \"{}\" }} }}) {{ uid }} }}", parent_id);
    let res = schema.execute_with_resolver(&query, resolver.clone()).await;
    let child_id = serde_json::from_str::<JsonValue>(&res).unwrap()["data"]["createChild"]["uid"].as_str().unwrap().to_string();

    // 3. Verify Forward Link (Child -> Parent)
    let query = format!("query {{ getChild(uid: \"{}\") {{ parent {{ id }} }} }}", child_id);
    let res = schema.execute_with_resolver(&query, resolver.clone()).await;
    ...
    let query = format!("query {{ getParent(uid: \"{}\") {{ children {{ id }} }} }}", parent_id);
    let res = schema.execute_with_resolver(&query, resolver.clone()).await;
    let json: JsonValue = serde_json::from_str(&res).unwrap();
    let children = json["data"]["getParent"]["children"].as_array().unwrap();
    
    assert_eq!(children.len(), 1, "Parent should have 1 child linked via inverse");
}
