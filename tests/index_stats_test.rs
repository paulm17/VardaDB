...
async fn test_index_stats() {
    ...
    let storage = std::sync::Arc::new(
        vardadb::storage::backend::Storage::new(db_path, Some(1))
    ...
    let schema =
        Schema::load_with_resolver(sdl, resolver.clone()).expect("Failed to load schema");
    ...
    let index_size_bytes = stats["indexSizeBytes"]
        .as_i64()
        .expect("indexSizeBytes should be i64");
    let segment_count = stats["segmentCount"]
        .as_i64()
        .expect("segmentCount should be i64");
