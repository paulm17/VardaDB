...
async fn test_cascade_delete() {
    ...
    // Assuming RedbResolver::new takes Storage (or Arc<Storage>?)
    // Let's check RedbResolver signature. Usually it takes Arc<Storage> or just Storage if lightweight.
    ...
    let resolver = Box::new(RedbResolver::new(std::sync::Arc::new(storage), "default"));
    ...
    ";
    let res_user = schema
        .execute_with_resolver(mutation_create_user, resolver.clone())
        .await;
    ...
    let res_post1 = schema
        .execute_with_resolver(&mutation_create_post1, resolver.clone())
        .await;
    let post1_json: JsonValue = serde_json::from_str(&res_post1).unwrap();
    let post1_id_str = post1_json["data"]["createPost"]["uid"]
        .as_str()
        .expect("Post1 ID")
        .to_string();
    ...
    let res_post2 = schema
        .execute_with_resolver(&mutation_create_post2, resolver.clone())
        .await;
    let post2_json: JsonValue = serde_json::from_str(&res_post2).unwrap();
    let post2_id_str = post2_json["data"]["createPost"]["uid"]
        .as_str()
        .expect("Post2 ID")
        .to_string();
    ...
    let res_check = schema
        .execute_with_resolver(&query_user, resolver.clone())
        .await;
    let check_val: JsonValue = serde_json::from_str(&res_check).unwrap();
    let posts = check_val["data"]["getUser"]["posts"]
        .as_array()
        .expect("Posts array");
    ...
    let res_del = schema
        .execute_with_resolver(&mutation_delete, resolver.clone())
        .await;
    ...
    let res_gone = schema
        .execute_with_resolver(&query_user_gone, resolver.clone())
        .await;
