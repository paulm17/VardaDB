    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    fn on_new_span(
        ...
        if let Some(span) = ctx.span(id) {
            ...
            data.start_ts = std::time::SystemTime::now()
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            for entry in flusher_counters.iter() {
                ...
                let _ = flusher_storage
                    .metrics_table
                    .insert(db_key.as_bytes(), &val.to_be_bytes());
            }
            ...
            for entry in flusher_gauges.iter() {
                ...
                let _ = flusher_storage
                    .metrics_table
                    .insert(db_key.as_bytes(), &val_bits.to_be_bytes());
            }
            ...
            for entry in flusher_histograms.iter() {
                ...
                let _ = flusher_storage
                    .metrics_table
                    .insert(db_key_p50.as_bytes(), &p50.to_be_bytes());

                let db_key_p90 = format!("h:{}:p90:{}", key_str, now_ts);
                let _ = flusher_storage
                    .metrics_table
                    .insert(db_key_p90.as_bytes(), &p90.to_be_bytes());
            }
            ...
            let _ = flusher_storage
                .metrics_table
                .insert(k_cpu.as_bytes(), &(cpu_usage as f64).to_be_bytes());

            let k_mem = format!("g:system.memory:{}", now_ts);
            let _ = flusher_storage
                .metrics_table
                .insert(k_mem.as_bytes(), &(mem_usage as f64).to_be_bytes());
        }
    });
}
