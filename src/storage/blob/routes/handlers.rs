...
pub async fn upload_chunk(
    ...
    let is_octet = check_header(&headers, "content-type", |v| {
        v == "application/offset+octet-stream"
    });
    if !is_octet {
        return Ok((
    ...
    if info.length == Some(info.offset) {
        if let Some(path_str) = &info.path {
            ...
            builder = builder.header(
                "Varda-File-Url",
                format!("/files/hash/{}", info.content_hash.clone().unwrap()),
            );
        }
    }
