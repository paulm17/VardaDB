...
pub trait DataStorage: Send + Sync {
    ...
    async fn get_contents(&self, file_info: &FileInfo)
        -> Result<Response<Body>, VardaStorageError>;
    async fn add_bytes(
        &self,
        file_info: &mut FileInfo,
        bytes: Bytes,
    ) -> Result<(), VardaStorageError>;
    ...
    async fn finalize(
        ...
        upload_id: &str,
        staging_path: &Path,
    ) -> Result<(PathBuf, String), VardaStorageError>;
}
...
impl DataStorage for VardaDataStorage {
    ...
    async fn add_bytes(
        ...
        let path = file_info
            .path
            .as_ref()
            .ok_or(VardaStorageError::FileNotFound)?;
