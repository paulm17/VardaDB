...
impl crate::engine::resolver::Resolver for DummyResolver {
    ...
    fn subscribe_events(&self) -> crate::realtime::bus::EventBus {
        crate::realtime::bus::EventBus::new()
    }
...
pub async fn init_system(config: crate::config::VardaConfig) -> (Arc<ServerState>, Router) {
    ...
    let storage = Arc::new(
        Storage::new(&storage_path, config.server.node_id).expect("Failed to open storage"),
    );
    ...
    let (sdl, _is_default) = match loaded_sdl {
        ...
        None => {
            println!(
                "No persisted schema found at {}. Using default.",
                schema_file_path
            );
    ...
    let initial_schema = crate::engine::schema::Schema::load_with_resolver(&sdl, resolver.clone())
        .or_else(|e| {
            println!(
                "Failed to load persisted schema: {}. Falling back to default.",
                e
            );
    ...
    schemas.insert(
        "default".to_string(),
        Arc::new(RwLock::new(Arc::new(initial_schema))),
    );
    ...
    let sync_schema = state
        .schemas
        .get("default")
        .expect("Default schema missing")
        .clone();
    let sync_cache = state.cache.clone();

    tokio::spawn(async move {
        println!("Initializing Zenoh Sync...");
        match crate::sync::manager::SyncManager::new(
            sync_resolver,
            zenoh_config,
            remote_append_path,
            sync_schema,
    ...
    let obs_router =
        crate::observability::router::routes::<ServerState>().with_state((*state).clone());

    // Initialize Blob Storage State
    let blob_state = Arc::new(
        crate::storage::blob::routes::BlobState::new(&config, state.clone())
            .await
    ...
    let app = Router::new()
        .route("/v1/chat/completions", post(crate::llm::chat_handler))
        .route("/v1/embeddings", post(crate::llm::embeddings_handler))
        .route("/v1/models", get(crate::llm::models_handler))
    ...
    let app = if state.auth.is_some() {
        ...
        let auth_router = Router::new()
            .route(
                "/auth/register",
                post(auth::handlers::register::register_user_handler),
            )
            .route(
                "/auth/login",
                post(auth::handlers::login::login_user_handler),
            )
            .route(
                "/auth/forgot_password",
                post(auth::handlers::forgot_password::forgot_password_handler),
            )
            .route(
                "/auth/reset_password",
                post(auth::handlers::reset_password::reset_password_handler),
            )
            .route(
                "/auth/generate_magiclink",
                post(auth::handlers::generate_magiclink::generate_magiclink_handler),
            )
            .route(
                "/auth/verify_magiclink_code",
                get(auth::handlers::verify_magiclink::verify_magiclink_code_handler),
            )
            .route(
                "/auth/check_code",
                post(auth::handlers::check_code::check_code_handler),
            )
            .route(
                "/auth/verify_code",
                get(auth::handlers::verify_code::verify_code_handler),
            )
            .route(
                "/auth/logout",
                get(auth::handlers::logout::logout_handler).route_layer(
            ...
            .route(
                "/auth/me",
                get(auth::handlers::get_me::get_me_handler).route_layer(
                    middleware::from_fn_with_state(
                        auth_state.clone(),
                        auth::middleware::auth_middleware,
                    ),
...
pub async fn run(config: crate::config::VardaConfig) {
    ...
    if is_mcp {
        ...
        println!(
            "GraphiQL playground at http://127.0.0.1:{}/playground",
            port
        );
        println!(
            "Admin Schema Endpoint at http://127.0.0.1:{}/admin/schema",
            port
        );
...
async fn graphql_handler(
    ...
    let schema = if let Some(s) = state.schemas.get(&db_name) {
        ...
        if state.storage.get_database(&db_name).is_some() {
            ...
            let sdl = std::fs::read_to_string(&db_schema_path)
                .unwrap_or_else(|_| "type Health { status: String }".to_string());
            ...
            println!(
                "Database {} not found/loaded, defaulting to default schema context.",
                db_name
            );
            if let Some(s) = state.schemas.get("default") {
                s.read().await.clone()
            } else {
                let mut resp = async_graphql::Response::new(async_graphql::Value::Null);
                resp.errors = vec![async_graphql::ServerError::new(
                    "Internal Error: Default schema missing",
                    None,
                )];
                return resp.into();
            }
            ...
            if let Some(entry) = state.schemas.get("default") {
                ...
                *lock = Arc::new(new_schema);
                ...
                state.schemas.insert(
                    "default".to_string(),
                    Arc::new(RwLock::new(Arc::new(new_schema))),
                );
            }
...
async fn subscription_handler(
    ...
    let _schema_wrapper = if let Some(s) = state.schemas.get(&db_name) {
        ...
        state
            .schemas
            .get("default")
            .expect("Default schema missing from map")
            .read()
            .await
            .clone()
    };
                        ...
                        if let Ok(val) = serde_json::from_str::<serde_json::Value>(s) {
                            ...
                            if val["type"] == "connection_init" {
                                if let Some(payload) = val.get("payload") {
                                    ...
                                    if let Some(d) = db {
                                        selected_db = d.to_string();
                                    } else if let Some(n) = ns {
                                        selected_db = n.to_string();
                                    }
                                }
                            }
            ...
            let schema_wrapper = if let Some(s) = state.schemas.get(&selected_db) {
                ...
                if state.storage.get_database(&selected_db).is_some() {
                    ...
                    if let Ok(new_schema) =
                        crate::engine::schema::Schema::load_with_resolver(&sdl, resolver)
                    {
                        let arc_schema = Arc::new(RwLock::new(Arc::new(new_schema)));
                        state
                            .schemas
                            .insert(selected_db.clone(), arc_schema.clone());
                        let x = arc_schema.read().await.clone();
                        x
                    } else {
                        state
