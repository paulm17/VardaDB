...
impl SyncManager {
    ...
    pub async fn start(&self) -> anyhow::Result<()> {
                ...
                match msg {
                     ...
                     SyncMessage::RequestSchema => {
                         ...
                         if has_user_types {
                             ...
                             let envelope = crate::sync::reconciliation::SyncEnvelope { sender: node_id, message: resp };
                     ...
                     SyncMessage::Gossip(remote_fp) | SyncMessage::RangeResponse(remote_fp) => {
                         ...
                         if let Ok(local_fp) = resolver_worker.compute_fingerprint_range(&remote_fp.start, &remote_fp.end) {
                             if local_fp.hash != remote_fp.hash {
                                 ...
                                 if local_fp.count <= THRESHOLD || remote_fp.count <= THRESHOLD || is_atomic {
                                     ...
                                     let envelope = crate::sync::reconciliation::SyncEnvelope { sender: node_id, message: req };
                                     if let Ok(payload) = serde_json::to_vec(&envelope) {
                                     ...
                                     let env1 = crate::sync::reconciliation::SyncEnvelope { sender: node_id, message: req1 };
                                     if let Ok(p1) = serde_json::to_vec(&env1) { let _ = network_worker.publish(&key, &p1).await; }
                                     
                                     let env2 = crate::sync::reconciliation::SyncEnvelope { sender: node_id, message: req2 };
                                     if let Ok(p2) = serde_json::to_vec(&env2) { let _ = network_worker.publish(&key, &p2).await; }
                                 }
                             }
                         }
                     },
        ...
        self.network.start_sync_listener(self.prefix.clone(), listener_node_id, move |msg| {
