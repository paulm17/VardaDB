...
impl TaskCenter {
    ...
    where
        F: Future<Output = anyhow::Result<()>> + 'static,
    {
        Self::with_current(|tc| tc.start_runtime(root_task_kind, runtime_name, root_future))
    }
    ...
    ///
    pub async fn cancel_tasks(kind: Option<TaskKind>) {
        Self::current().cancel_tasks(kind).await
    }
...
impl TaskCenterInner {
        ...
        F: Future<Output = anyhow::Result<()>> + Send + 'static,
    {
        ...
        let (parent_id, parent_name) = self.with_task_context(|ctx| (ctx.id, ctx.name.clone()));
        ...
        let result = self.spawn_inner(kind, name.clone(), parent_id, cancel, future);
    ...
    where
        ...
        T: Send + 'static,
    {
        let (parent_id, parent_name, is_parent_cancelled) = self.with_task_context(|ctx| {
            (
                ctx.id,
                ctx.name.clone(),
        ...
        F: Future<Output = anyhow::Result<()>> + 'static,
    {
        ...
        let inner_handle = tokio::task::spawn_local(fut);
        ...
        T: Send + 'static,
    {
        ...
        let inner_handle = runtime.spawn(fut);
    ...
    async fn shutdown_node_inner(self: &Arc<Self>, reason: &str, exit_code: i32) {
        ...
        self.cancel_tasks(Some(TaskKind::ClusterController)).await;
        // stop accepting ingress
        self.cancel_tasks(Some(TaskKind::HttpIngressRole)).await;
        // stop admin server and in-flight query-server requests
        self.cancel_tasks(Some(TaskKind::AdminApiServer)).await;
        // Worker will shutdown running processors
        self.cancel_tasks(Some(TaskKind::WorkerRole)).await;

        self.initiate_managed_runtimes_shutdown();
        // Ask bifrost to shutdown providers and loglets
        self.cancel_tasks(Some(TaskKind::BifrostBackgroundLowPriority))
            .await;
        self.cancel_tasks(Some(TaskKind::BifrostWatchdog)).await;

        // Stop log-server role
        self.cancel_tasks(Some(TaskKind::LogServerRole)).await;

        // stop metadata server
        self.cancel_tasks(Some(TaskKind::MetadataServer)).await;

        self.shutdown_managed_runtimes();
        // global shutdown trigger
        self.root_task_context.cancel();
        self.cancel_tasks(None).await;
    ...
    ///   cancel_tasks(Some(TaskKind::MetadataBackgroundSync), None)
    ///
    async fn cancel_tasks(self: &Arc<Self>, kind: Option<TaskKind>) {
...

        {
            ...
            for task in tasks.values() {
                if kind.is_none() || Some(task.context.kind) == kind {
                    ...
                    victims.push((Arc::clone(task), task.kind()));
                }
            }
        }

        for (task, task_kind) in victims {
            let handle = {
                let mut task_mut = task.handle.lock();
            ...
            if let Some(mut handle) = handle {
                if task_kind.should_abort_on_cancel() {
                    ...
                    debug!(kind = ?task_kind, name = ?task.name(), "task {} aborted!", task.id());
                    handle.abort();
                    ...
                    if tokio::time::timeout(Duration::from_secs(2), &mut handle)
                        ...
                        .is_err()
                    {
                        info!(kind = ?task_kind, name = ?task.name(), "waiting for task {} to shutdown", task.id());
