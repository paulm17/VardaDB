...
static HOSTNAME: LazyLock<String> = LazyLock::new(|| {
    std::panic::catch_unwind(hostname::get)
        .ok()
        .and_then(Result::ok)
...
pub struct CommonOptions {
    ...
    ///
    /// auto-provision is allowed by default in development mode and is disabled if Restate runs with `--production`
    ...
    /// Address to bind for the tokio-console tracing subscriber. If unset and Restate is
