...
use crate::invocation_task::{
    ...
    ResponseChunk, ResponseStream, TerminalLoopState, X_RESTATE_RUNTIME,
    ...
    Schemas: InvocationTargetResolver,
{
    ...
    fn handle_response_headers(
        ...
        if let Some(hv) = parts.headers.remove(X_RESTATE_RUNTIME) {
            self.invocation_task
                .send_invoker_tx(InvocationTaskOutputInner::ServerHeaderReceived(
                    hv.to_str()
                        .map_err(|e| InvokerError::BadHeader(X_RESTATE_RUNTIME, e))?
                        .to_owned(),
                ))
        }
