...
#[allow(clippy::declare_interior_mutable_const)]
const X_RESTATE_RUNTIME: HeaderName = HeaderName::from_static("x-restate-runtime");
...
where
    EE: EntryEnricher + Send + 'static,
    Schemas: DeploymentResolver + InvocationTargetResolver + Send + Sync + 'static,
{
    ...
    where
        IR: InvocationReader + Send + 'static,
    {
        Box::pin(async move {
