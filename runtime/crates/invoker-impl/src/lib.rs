    ...
    Schemas: DeploymentResolver + InvocationTargetResolver + Clone + Send + Sync + 'static,
{
    fn start_invocation_task(
        ...
        task_pool
            ...
            .spawn(async move { task.run(input_journal, storage_reader).await })
            .expect("to spawn invocation task")
    }
}

/// Currently the invoker id is mainly used with invoker specific metrics.
    ...
    pub async fn run(self, mut updateable_options: impl LiveLoad<Live = InvokerOptions>) {
            ...
            .await
        {
            ...
            Err(e) if e.kind() == ErrorKind::PermissionDenied => {
                ...
                Make sure the runtime has permissions to write that directory, or change the spill queue directory with the config option 'worker.invoker.tmp_dir' or the env RESTATE_WORKER__INVOKER__TMP_DIR.\n{e}",
                    tmp_dir.display()
                );
    ...
    Schemas: InvocationTargetResolver,
{
    ...
    async fn step(
        ...
        tokio::select! {
            ...
            Some(input_message) = self.input_rx.recv() => {
                match input_message {
                    ...
                    InputCommand::VQInvoke(command) => {
                        counter!(INVOKER_ENQUEUE, "status" => TASK_OP_COMPLETED).increment(1);
                        self.handle_vqueue_invoke(options, *command);
                    },
                    // --- Other commands (they don't go through the segment queue)
                    InputCommand::RegisterRuntime { partition_key_range, storage_reader, sender, } => {
                        self.handle_register_runtime(partition_key_range, storage_reader, sender);
                    },
                    InputCommand::Abort { invocation_id } => {
                    ...
                    InputCommand::Completion { invocation_id, completion } => {
                        self.handle_completion(invocation_id, completion);
                    },
                    InputCommand::Notification { invocation_id, notification } => {
                        self.handle_notification(options, invocation_id, notification);
                    },
            ...
            Some(invoke_input_command) = segmented_input_queue.next(), if !segmented_input_queue.inner().is_empty() && self.quota.is_slot_available() => {
                self.handle_invoke(options, invoke_input_command.invocation_id, invoke_input_command.invocation_target, invoke_input_command.journal);
            },
            Some(invocation_task_msg) = self.invocation_tasks_rx.recv() => {
                ...
                match inner {
                    ...
                    InvocationTaskOutputInner::Failed(e) => {
                        self.handle_invocation_task_failed(invocation_id, e).await
                    },
                    ...
                    InvocationTaskOutputInner::SuspendedV2(notification_ids) => {
                        self.handle_invocation_task_suspended_v2(invocation_id, notification_ids).await
                    }
                };
            },
    ...
    fn handle_register_runtime(
        &mut self,
        partition_key_range: RangeInclusive<PartitionKey>,
        storage_reader: IR,
        sender: mpsc::Sender<Box<Effect>>,
    ) {
        self.invocation_state_machine_manager.register_runtime(
            partition_key_range,
            storage_reader,
            sender,
        );
    ...
    fn handle_vqueue_invoke(&mut self, options: &InvokerOptions, command: VQueueInvokeCommand) {
        if self.invocation_state_machine_manager.has_runtime() {
            ...
            trace!(
                "No registered runtime was found for the invocation {}",
                command.invocation_id
            );
        }
    }
    ...
    fn handle_invoke(
        ...
        if self.invocation_state_machine_manager.has_runtime() {
            ...
            trace!("No registered runtime was found for the invocation {invocation_id}");
        }
    }

    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id))]
    ...
    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id, restate.journal.command.index = command_index))]
    ...
    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id, restate.deployment.id = %pinned_deployment.deployment_id))]
    ...
    fn handle_server_header_received(
        ...
        self.invocation_state_machine_manager
            .handle_for_invocation(&invocation_id, |_, ism| {
                trace!(
                    restate.invocation.target = %ism.invocation_target,
                    "runtime response header {}. Invocation state: {:?}",
                    runtime_header,
                    ism.invocation_state_debug()
                );

                self.status_store
                    .on_server_header_receiver(&invocation_id, runtime_header);
            });
    }

    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id, restate.journal.index = entry_index, restate.journal.entry_type = ?entry.ty()))]
    ...
    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id, restate.journal.entry.ty = %notification.ty(), restate.journal.notification.id = ?notification.id()))]
    ...
    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id, restate.journal.command.index = command_index, restate.journal.entry.ty = %command.ty()))]
    ...
    #[instrument(level = "trace", skip_all, fields(restate.invocation.id = %invocation_id))]
    fn handle_completion(&mut self, invocation_id: InvocationId, completion: Completion) {
        self.invocation_state_machine_manager
            .handle_for_invocation(&invocation_id, |_, ism| {
                trace!(
                    restate.invocation.target = %ism.invocation_target,
    ...
    #[instrument(level = "debug", skip_all, fields(restate.invocation.id = %invocation_id))]
    ...
    fn handle_abort_all(&mut self) {
        if let Some(invocation_state_machines) =
            self.invocation_state_machine_manager.remove_runtime()
        {
            ...
            trace!("Ignoring AbortAll command because there is no registered runtime");
        }
    }
    ...
    async fn handle_error_event(
        ...
        match result {
            OnTaskError::Retrying(next_retry_timer_duration) => {
                counter!(INVOKER_INVOCATION_TASKS, "status" => TASK_OP_FAILED, "transient" => "true")
                    .increment(1);
            ...
            OnTaskError::Pause => {
                counter!(INVOKER_INVOCATION_TASKS, "status" => TASK_OP_FAILED, "transient" => "false")
                    .increment(1);
                warn_it!(
                    error,
                    restate.invocation.id = %invocation_id,
...
mod tests {
        ...
        Schemas: InvocationTargetResolver,
    {
        ...
        fn register_mock_runtime(&mut self, storage_reader: IR) -> mpsc::Receiver<Box<Effect>>
        where
            ITR: InvocationTaskRunner<IR>,
        {
            let (runtime_tx, runtime_rx) = mpsc::channel(1024);
            self.handle_register_runtime(RangeInclusive::new(0, 0), storage_reader, runtime_tx);
            runtime_rx
        }
    ...
    async fn input_order_is_maintained() {
        ...
        handle
            .register_runtime(RangeInclusive::new(0, 0), EmptyStorageReader, output_tx)
            .unwrap();
        handle
            .invoke(
                invocation_id,
    ...
    async fn quota_allows_one_concurrent_invocation() {
        ...
        let _ = service_inner.register_mock_runtime(EmptyStorageReader);
    ...
    async fn notification_triggers_retry() {
        ...
        service_inner.handle_notification(&invoker_options, invocation_id, notification);
    ...
    async fn status_store_clears_last_failure_on_new_command() {
        ...
        let _effects_rx = service_inner.register_mock_runtime(EmptyStorageReader);
    ...
    async fn transient_error_event_deduplication() {
        ...
        let mut effects_rx = service_inner.register_mock_runtime(EmptyStorageReader);
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, error_a)
            .await;
        assert_that!(
            *effects_rx
                .try_recv()
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, error_a_same)
            .await;
        assert!(
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, error_b)
            .await;
        assert_that!(
            *effects_rx
                .try_recv()
    ...
    async fn abort_error_counts_towards_retry_policy() {
        ...
        let (_, _status_tx, mut service_inner) =
            ServiceInner::mock((), MockSchemas::default(), None);
        let _rx = service_inner.register_mock_runtime(EmptyStorageReader);
    ...
    async fn retry_iter_updates_on_pinned_deployment_and_not_reset_on_same_dp() {
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, err1)
            .await;
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, err2)
            .await;
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, err3)
            .await;
    ...
    async fn manual_pause_while_waiting_retry() {
        ...
        service_inner
            .handle_invocation_task_failed(invocation_id, error)
            .await;
        // Drain any proposed event
        let _ = effects_rx.try_recv();
