...
fn load_root_cert_store() -> rustls::RootCertStore {
    let certs = rustls_native_certs::load_native_certs();

    if !certs.errors.is_empty() {
        warn!(
            errors = ?certs.errors,
            "Native root certificate loading reported errors"
        );
    }
    ...
    if !certs.certs.is_empty() {
        ...
        if valid_count > 0 {
            if invalid_count > 0 {
                warn!(
                    invalid_count,
                    "Ignored invalid native root certificates while building the TLS root store"
                );
            }
            return roots;
        }
    ...
    warn!("No native root certificates found, falling back to bundled webpki roots");
